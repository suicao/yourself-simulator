...
class BucketSampler(Sampler):
    ...
    def __init__(self, data_source, batch_size, bucket_size=50,
                 droplast=False, shuffle=True):
        self._lens = data_source.lens
    ...
    def __iter__(self):
        ids = np.arange(len(self._lens))
        if self._shuf:
            np.random.shuffle(ids)
        buckets = [sorted(ids[i:i + self._bucket_size],
                          key=lambda i: self._lens[i], reverse=False)
                   for i in tqdm(range(0, len(ids), self._bucket_size), desc="Making buckets")]
        batches = [bucket[i:i + self._batch_size]
                   for bucket in tqdm(buckets, desc="Making batches")
...
class Trainer:
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        #         print(f"Distributed world size: {dist.get_world_size()}")
        train_sampler = (
            RandomSampler(self.train_dataset) if self.args.local_rank == -1 else DistributedSampler(self.train_dataset)
            #            BucketSampler(self.train_dataset,self.args.train_batch_size) if self.args.local_rank == -1 else DistributedSampler(self.train_dataset)
        )
        return DataLoader(
            self.train_dataset,
